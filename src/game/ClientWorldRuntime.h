...
namespace dolbuto::game
{
    ...
    public:
        ...
        world::ChunkPrepareSystem chunkPrepareSystem;
        ...
        RuntimeChunk& requestLightResolveTicket(int chunkX, int chunkZ, uint64_t generation, uint32_t priority, const ChunkLoadEnqueue& enqueueChunkLoad);
        RuntimeChunk& requestLocalLightTicket(int chunkX, int chunkZ, uint64_t generation, uint32_t priority, const ChunkLoadEnqueue& enqueueChunkLoad);
        std::optional<TerrainJob> makeTerrainSourceJobIfNeeded(RuntimeChunk& chunk, uint64_t generation);
        std::optional<TerrainJob> makeLocalLightJobIfReady(uint64_t key, uint64_t generation);
        std::optional<TerrainJob> makeLightResolveJobIfReady(uint64_t key, uint64_t generation);
        ...
        static RuntimeChunk prepareRuntimeChunkFromSnapshot(const SaveChunkSnapshot& snapshot, uint64_t generation, const LightAttenuationTables* lightAttenuation);
