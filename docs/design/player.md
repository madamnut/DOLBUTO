...
- 좌클릭: 선택 블록 파괴
- 우클릭: 바라보는 드랍 아이템에 손 아이템의 상호작용 후보가 있으면 후보 UI를 열고, 후보가 없으면 이전 위치에 현재 배치 모드 블록 배치
...
내구도가 있는 아이템이 실제로 블록을 파괴하면 동작이 맞을 때 내구도 1, 동작이 다를 때 내구도 3을 소비한다.
