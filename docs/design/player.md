...
- 좌클릭: 선택 블록 파괴
- 우클릭: 바라보는 드랍 아이템에 손 아이템의 상호작용 후보가 있으면 원형 후보 UI를 열고, 후보가 없으면 이전 위치에 현재 배치 모드 블록 배치
...
드랍 아이템 상호작용은 손에 든 아이템의 `actions`와 `assets/data/interactions.json`의 `action`/`target` 항목을 기준으로 후보를 찾는다.
우클릭으로 후보가 열리면 카메라 회전과 플레이어 이동을 멈추고 마우스 커서를 화면 중앙에 둔다.
중앙 미선택 영역 밖으로 마우스를 움직이면 중간 링에서는 액션을, 바깥 링에서는 선택 액션의 후보를 중심 기준 각도로 선택한다.
우클릭을 떼면 선택 액션과 선택 후보 조합을 실행한다.
선택 없이 중앙 영역에서 우클릭을 떼거나 `Esc`를 누르면 취소한다.
...
- `Shift`: ground 웅크리기
- `F`: 드랍 아이템 획득
- `우클릭 유지`: 가능한 드랍 아이템 상호작용 후보 원형 UI 열기
- `Q`: 선택 핫바 아이템 1개 버리기
